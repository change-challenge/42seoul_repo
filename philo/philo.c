...
size_t	get_time()
{
	...
	if(gettimeofday(&time, NULL) == ERROR) // 4가지 settimeofday => gettimeofday -> 
		return (ERROR);
...
int	take_fork(t_philo *philo)
{
	...
	if (philo->idx % 2 == 0)
	{
		...
		if (philo_print(philo, philo->info, philo->idx, "has taken a fork") == ERROR)
			return (ERROR);
