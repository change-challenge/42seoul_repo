...
/*   Updated: 2022/03/09 14:44:44 by hchang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*make_str_x(size_t arg, t_gather *fwp, int len, int full_len)
{
	char	*result;

	if ((fwp->bits & FG_ZERO) && (salloc_int(&result, full_len, '0') == ERROR))
		return (NULL);
	else if (!(fwp->bits & FG_ZERO) && \
		(salloc_int(&result, full_len, ' ') == ERROR))
		return (NULL);
	if (fwp->bits & FG_MINUS)
		input_hex_base(result + isfg_inc(fwp), arg, len, fwp);
	else
		input_hex_base(result + full_len - len, arg, len, fwp);
